...
> 最後更新：2026-07-03（v2.1：加入 Look-ahead Bias 警示）  
> 規格書：https://raw.githubusercontent.com/whocare-lab/stock/main/event_study_zscore_v2.md

---

## ⚠️ 重要警示：分組分析的 Look-ahead Bias 問題

> **每次做分組分析前，必須問自己：這個分組依據，在公布前就能知道嗎？**

### 什麼是 Look-ahead Bias（前視偏誤）

在事件研究法中，若使用「事件發生後才能知道的資訊」作為分組依據，去解釋「事件發生前的股價行為」，即為 Look-ahead Bias，統計結果會失去因果詮釋力。

### 本分析的錯誤案例

**錯誤做法（已廢棄）**：以月營收 YoY 年增率（> 0% = Beat，≤ 0% = Miss）分組。  
**問題**：YoY 年增率在公布當天才知道，公布前無法得知，屬於事後資訊。  
**後果**：Beat 組洩漏期出現 −3.87% 顯著負異常（p<0.05），但此結果是偽陽性，在真實交易中無法實現。

**正確做法**：以市場參與者「公布前即已知曉」的外生變數分組。

### 合法的分組依據（事前可知）

| 分組方式 | 說明 | 適用場景 |
|---|---|---|
| **季節旺淡季** | Q2+Q3（旺季）vs Q1+Q4（淡季），月份為公開資訊 | 半導體、消費電子 |
| **公司規模** | 大型股 vs 中小型股，以市值為準 | 跨公司橫截面 |
| **市場狀態** | 多頭期（指數 > MA200）vs 空頭期 | 市場環境效應 |
| **分析師預估共識** | 僅限有 Consensus EPS/Revenue 的個股（美股為主） | 機構覆蓋度高的標的 |

### 季節分組分析結果（2026-07-03 修正版）

修正為旺淡季分組後，N=42 的中美晶事件分析結果：
- 旺季（Q2+Q3，N=21）洩漏期 CAAR = −2.21%，p = 0.211（**不顯著**）
- 淡季（Q4+Q1，N=21）洩漏期 CAAR = −0.38%，p = 0.853（**不顯著**）
- 組間差異 Welch t-test：p = 0.493（**不顯著**）
- **結論：季節分組對股價異常報酬無顯著解釋力，市場效率假說獲支持**

詳細結果見：[research_findings_seasonal.md](https://raw.githubusercontent.com/whocare-lab/stock/main/research_findings_seasonal.md)
