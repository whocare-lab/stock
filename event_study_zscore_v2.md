...
>
> **重要提醒：** 本規格書為元智大學 DBA 博士班課程中，針對事件研究法（Event Study）所延伸的知識性學習與程式驗證範例，主要用於研究、教學與模型測試，**不構成任何真實股票投資建議**。
...
對每個事件（營收公布），先輸出一個給人快速判讀的四欄結果表：

| 前窗 | 後窗 | CAR | 結論 |
|---|---|---|---|
| T-10 ~ T-1 的報酬 / 顯著性 | T+1 ~ T+10 的報酬 / 顯著性 | CAR(前窗)、CAR(後窗)、ΔCAR | event_before_buy / event_after_buy / no_signal |

### 欄位說明

- **前窗**：事件前 10 個交易日的表現，觀察是否已有提前反應
- **後窗**：事件後 10 個交易日的表現，觀察事件後是否還有延續
- **CAR**：至少列出前窗 CAR、後窗 CAR，以及兩者差額 ΔCAR
- **結論**：根據前後窗比較，給出「事件前買」「事件後買」或「無明確訊號」

### 建議輸出規則

1. 若前窗 CAR 顯著為正，通常代表訊息可能已提前反應，事件前進場優勢下降。
2. 若後窗 CAR 顯著為正、前窗未明顯洩漏，通常代表事件後進場較有利。
3. 若前後兩窗都不顯著，視為 no signal，不建議只靠這個事件下單。
4. Z-Score 仍可保留，但只當作顯著性檢查，不作為單獨買點判斷。
